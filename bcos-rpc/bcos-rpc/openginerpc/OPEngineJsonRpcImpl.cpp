...
void OPEngineJsonRpcImpl::onRPCRequest(std::string_view _requestBody, const Sender& _sender)
{
    ...
    try
    {
        ...
        {
            OPENGINE_LOG(TRACE) << LOG_BADGE("onRPCRequest") << LOG_DESC("begin")
    ...
    if (c_fileLogLevel == DEBUG) [[unlikely]]
    {
        ...
        OPENGINE_LOG(DEBUG) << LOG_BADGE("onRPCRequest") << LOG_DESC("end")
                            << LOG_KV("request", _requestBody)
    ...
    Json::Value _request, const std::function<void(Json::Value)>& _callback)
{
    ...
    try
    {
        ...
        if (c_fileLogLevel == TRACE) [[unlikely]]
        {
            OPENGINE_LOG(TRACE) << LOG_BADGE("handleRequest") << LOG_DESC("begin")
...
void OPEngineJsonRpcImpl::handleBatchRequest(Json::Value _request, const Sender& _sender)
{
    ...
    if (c_fileLogLevel == TRACE) [[unlikely]]
    {
        OPENGINE_LOG(TRACE) << LOG_BADGE("handleBatchRequest") << LOG_DESC("begin")
    ...
    for (auto& req : _request)
    {
        handleRequest(std::move(req), [responses, requestSize, _sender, startT](Json::Value _response) {
            ...
            {
                auto const endT = utcTime();
                OPENGINE_LOG(TRACE) << LOG_BADGE("handleBatchRequest") << LOG_DESC("end")
                                    << LOG_KV("costMs", endT - startT)
