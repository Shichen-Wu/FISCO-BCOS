...
public:
    ...
    virtual void updateFinishedState(MVBAMessageInterface::Ptr _notifyFinishedMsg);

    //处理已完成Cache，TODO：更新MVBA的epoch状态
    virtual void handleFinishedCache(EpochIndexType _index);
    
    //随机选择leader
    virtual IndexType calculateElectedLeader(const bcos::crypto::HashType& _proposalHash) const;
