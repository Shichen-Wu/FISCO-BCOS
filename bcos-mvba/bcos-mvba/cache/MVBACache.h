...
public:
    ...
    virtual void addActiveCache(MVBAMessageInterface::Ptr _activeMsg)
    {
        ...
        if (!m_activeProposal)
        {
            ...
            {
                MVBA_LOG(ERROR) << LOG_DESC("addActiveCache: createActiveMessage failed")
        ...
        if (activeEchoMsg)
        {
            ...
            else
            {
                ...
                {
                    MVBA_LOG(ERROR) << LOG_DESC("addActiveCache: can't find nodeInfo for generatedFrom")
        ...
        {
            MVBA_LOG(ERROR) << LOG_DESC("addActiveCache: createActiveEchoMessage failed")
                            << LOG_KV("index", m_index) << m_config->printCurrentState();
        }
    ...
    virtual void addLockCache(MVBAMessageInterface::Ptr _lockMsg)
    {
        ...
        if (lockEchoMsg)
        {
            ...
            else
            {
                ...
                {
                    MVBA_LOG(ERROR) << LOG_DESC("addLockCache: can't find nodeInfo for generatedFrom")
                                    << LOG_KV("generatedFrom", generatedFrom)
                                    << LOG_KV("index", m_index);
                }
            }
        ...
        {
            MVBA_LOG(ERROR) << LOG_DESC("addLockCache: createLockEchoMessage failed")
                            << LOG_KV("index", m_index) << m_config->printCurrentState();
        }
    ...
    // 检查并尝试结束lock阶段：当收到的lockEchoWeight超过了阈值，也即collectEnoughLockEcho返回true，则将m_locked设为1，create finish消息，广播finish消息，并将finish存入myfinish，并返回true；
    ...
    //virtual MVBAMessageInterface::Ptr finishMessage() { return m_finishMessage; }
    
    bool isActived() const { return m_actived; }
    bool isLocked() const { return m_locked; }
    bool isFinished() const { return m_finished; }
    
    MVBAProposalInterface::Ptr activeProposal() const { return m_activeProposal; }
    ...
    virtual void setSignatureList(
        MVBAProposalInterface::Ptr _proposal, CollectionCacheType& _cache);
    ...
    void registerLockNotify(std::function<void(bcos::protocol::BlockNumber)> _notifier)
    {
        m_lockNotifier = std::move(_notifier);
    }
    
    void registerFinishNotify(std::function<void(bcos::protocol::BlockNumber)> _notifier)
    {
...
protected:
    ...
    virtual MVBAMessageInterface::Ptr createActiveEchoMessage(MVBAMessageInterface::Ptr, bcos::crypto::HashType);

    // 根据收到的lock创建lockEcho消息，具体计算lock的hash值并放入lockEcho的payload，然后生成本节点的签名和完整消息
    virtual MVBAMessageInterface::Ptr createLockEchoMessage(MVBAMessageInterface::Ptr, bcos::crypto::HashType);
