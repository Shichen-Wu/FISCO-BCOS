...
class MVBABaseMessage : virtual public MVBABaseMessageInterface {
  ...
  void setPacketType(MVBAPacketType _packetType) override {
  ...
  // 序列化和反序列化
  bytesPointer encode(bcos::crypto::CryptoSuite::Ptr,
                      bcos::crypto::KeyPairInterface::Ptr) const override {
    return bcos::protocol::encodePBObject(m_baseMessage);
  }

  bytesPointer encode() const {
    return bcos::protocol::encodePBObject(m_baseMessage);
  }
  ...
  bytesConstRef signatureData() override {
    ...
    return bytesConstRef((byte const *)signatureData.data(),
                         signatureData.size());
  }

  bcos::crypto::HashType const &signatureDataHash() override {
    return m_dataHash;
  }
  ...
  void setSignatureData(bytes const &_signatureData) override {
    m_baseMessage->set_signaturedata(_signatureData.data(),
                                     _signatureData.size());
  }

  void setSignatureDataHash(bcos::crypto::HashType const &_hash) override {
    m_dataHash = _hash;
    m_baseMessage->set_signaturehash(_hash.data(),
                                     bcos::crypto::HashType::SIZE);
  }

  bool verifySignature(bcos::crypto::CryptoSuite::Ptr _cryptoSuite,
                       bcos::crypto::PublicPtr _pubKey) override {
    return _cryptoSuite->signatureImpl()->verify(_pubKey, signatureDataHash(),
                                                 signatureData());
  }

  uint64_t index() const override {
    return static_cast<uint64_t>(m_baseMessage->index());
  }
  ...
  bool operator==(MVBABaseMessage const &_mvbaMessage) const {
    return (timestamp() == _mvbaMessage.timestamp()) &&
           ...
           (view() == _mvbaMessage.view()) && (hash() == _mvbaMessage.hash()) &&
  ...
  uint64_t liveTimeInMilliseconds() const override {
    return bcos::utcTime() - m_createTime;
  }

  std::string toDebugString() const override {
    std::stringstream stringstream;
    stringstream << LOG_KV("type", m_packetType) << LOG_KV("round", m_round)
  ...
  virtual void deserializeToObject() {
    ...
    if (hashData.size() >= bcos::crypto::HashType::SIZE) {
      m_hash = bcos::crypto::HashType((byte const *)hashData.c_str(),
                                      bcos::crypto::HashType::SIZE);
    }
    ...
    if (signatureDataHash.size() >= bcos::crypto::HashType::SIZE) {
      m_dataHash =
          bcos::crypto::HashType((byte const *)signatureDataHash.c_str(),
                                 bcos::crypto::HashType::SIZE);
    }
  }

  std::shared_ptr<MVBARawBaseMessage> baseMessage() { return m_baseMessage; }
  void setBaseMessage(std::shared_ptr<MVBARawBaseMessage> _baseMessage) {
